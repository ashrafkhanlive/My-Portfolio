...
export function getConfiguredProjectBySlug(slug: string): Project | null {
...
export function getProjectTechnologies(project: Project): string[] {
  ...
  return Array.from(new Set(techGroups.flat()));
}

export function getConfiguredProjectCaseStudyBySlug(
  slug: string,
): ProjectCaseStudy | null {
  const project = getConfiguredProjectBySlug(slug);
  ...
  return {
    ...
    frontmatter: {
      ...
      image: project.image ?? '/project/valorant.png',
      technologies,
      techStack: project.techStack,
      github: project.github ?? '',
      live: project.live,
      timeline: 'Project build',
      role: project.categories[0] ?? 'Developer',
      status: project.isWorking ? 'completed' : 'in-progress',
      featured: project.badges.includes('Featured'),
      isPublished: true,
    },
...
export function getAllProjectPageSlugs(): string[] {
  return Array.from(
    new Set([
      ...getProjectCaseStudySlugs(),
      ...projects.map((project) => getProjectSlug(project)),
    ]),
  );
}
